...
class GroupVerticalSelfAttentionLayer(nn.Module):
    def __init__(self, dim,  num_heads=8, attn_drop=0., proj_drop=0., drop_path=0., dropout=0.):
        ...
        self.block = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr, sr_ratio=sr_ratio, ws=ws, with_rpe=True, vert_c_dim=64, groupattention=True)
...
class VerticalSelfAttentionLayer(nn.Module):
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., drop_path=0., dropout=0.):
        ...
        self.local_block = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr, sr_ratio=sr_ratio, ws=ws, with_rpe=True, vert_c_dim=64)
        self.global_block = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr, sr_ratio=sr_ratio, ws=1, with_rpe=True, vert_c_dim=64)
...
class CostPerceiverEncoder(nn.Module):
    def __init__(self):
        ...
        self.patch_size = 8
        self.patch_embed = PatchEmbed(in_chans=1, patch_size=self.patch_size, embed_dim=64, pe='linear')

        self.depth = 3

        self.latent_tokens = nn.Parameter(torch.randn(1, 8, 128))

        query_token_dim, tgt_token_dim = 128, 64*2
        qk_dim, v_dim = query_token_dim, query_token_dim
        self.input_layer = CrossAttentionLayer(qk_dim, v_dim, query_token_dim, tgt_token_dim, dropout=0)

        self.encoder_layers = nn.ModuleList([SelfAttentionLayer(128,  dropout=0) for idx in range(self.depth)])

        self.vertical_encoder_layers = nn.ModuleList([VerticalSelfAttentionLayer(128, dropout=0) for idx in range(self.depth)])
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        if self.cost_scale_aug is not None:
            scale_factor = torch.FloatTensor(B*H1*W1, 1, H2, W2).uniform_(self.cost_scale_aug[0], self.cost_scale_aug[1]).cuda()
        ...
        for idx, layer in enumerate(self.encoder_layers):
            x = layer(x)
            x = x.view(B, H1*W1, 8, -1).permute(0, 2, 1, 3).reshape(B*8, H1*W1, -1)
            x = self.vertical_encoder_layers[idx](x, (H1, W1), context)
            x = x.view(B, 8, H1*W1, -1).permute(0, 2, 1, 3).reshape(B*H1*W1, 8, -1)
