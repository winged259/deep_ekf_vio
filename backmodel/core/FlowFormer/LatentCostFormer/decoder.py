...
class MemoryDecoderLayer(nn.Module):
    def __init__(self, dim):
        ...
        self.patch_size = 8 # for converting coords into H2', W2' space

        query_token_dim, tgt_token_dim = 64, 128
        qk_dim, v_dim = query_token_dim, query_token_dim
        self.cross_attend = CrossAttentionLayer(qk_dim, v_dim, query_token_dim, tgt_token_dim, add_flow_token=True, dropout=0)
...
class MemoryDecoder(nn.Module):
    def __init__(self):
        ...
        self.flow_token_encoder = nn.Sequential(
            nn.Conv2d(81*1, dim, 1, 1),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1, 1)
        )
        self.proj = nn.Conv2d(256, 256, 1)
        self.depth = 12
        self.decoder_layer = MemoryDecoderLayer(dim)
        
        self.update_block = GMAUpdateBlock( hidden_dim=128)
