...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, ws=None, with_rpe=False, vert_c_dim=0, groupattention=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        if groupattention:
            assert with_rpe, "Not implementing groupattention without rpe"
            if vert_c_dim > 0:
                self.attn = GroupAttnRPEContext(dim, num_heads, attn_drop, drop, ws, vert_c_dim)
            else:
                self.attn = GroupAttnRPE(dim, num_heads, attn_drop, drop, ws)
        elif ws is None:
