...
from .depth import ResnetEncoder
...
class PWC_tf(nn.Module):
    def __init__(self, md=4):
        ...
        dd = np.array([512,256,128,64,64])

        od = nd
        self.conv6_0 = conv(od,      512, kernel_size=3, stride=1)
        self.conv6_1 = conv(dd[0],   256, kernel_size=3, stride=1)
        self.conv6_2 = conv(dd[0]+dd[1],128,  kernel_size=3, stride=1)
        self.conv6_3 = conv(dd[1]+dd[2],64,  kernel_size=3, stride=1)
        self.conv6_4 = conv(dd[2]+dd[3],64,  kernel_size=3, stride=1)        
        ...
        od = nd+256+2
        self.conv5_0 = conv(od,      512, kernel_size=3, stride=1)
        self.conv5_1 = conv(dd[0],   256, kernel_size=3, stride=1)
        self.conv5_2 = conv(dd[0]+dd[1],128,  kernel_size=3, stride=1)
        self.conv5_3 = conv(dd[1]+dd[2],64,  kernel_size=3, stride=1)
        self.conv5_4 = conv(dd[2]+dd[3],64,  kernel_size=3, stride=1)
        self.predict_flow5 = self.predict_flow(dd[3]+dd[4]) 
        #self.deconv5 = deconv(2, 2, kernel_size=4, stride=2, padding=1) 
        #self.upfeat5 = deconv(od+dd[4], 2, kernel_size=4, stride=2, padding=1) 
        
        od = nd+128+2
        self.conv4_0 = conv(od,      512, kernel_size=3, stride=1)
        self.conv4_1 = conv(dd[0],   256, kernel_size=3, stride=1)
        self.conv4_2 = conv(dd[0]+dd[1],128,  kernel_size=3, stride=1)
        self.conv4_3 = conv(dd[1]+dd[2],64,  kernel_size=3, stride=1)
        self.conv4_4 = conv(dd[2]+dd[3],64,  kernel_size=3, stride=1)
        self.predict_flow4 = self.predict_flow(dd[3]+dd[4]) 
        #self.deconv4 = deconv(2, 2, kernel_size=4, stride=2, padding=1) 
        #self.upfeat4 = deconv(od+dd[4], 2, kernel_size=4, stride=2, padding=1) 
        
        od = nd+64+2
        self.conv3_0 = conv(od,      512, kernel_size=3, stride=1)
        self.conv3_1 = conv(dd[0],   256, kernel_size=3, stride=1)
        self.conv3_2 = conv(dd[0]+dd[1],128,  kernel_size=3, stride=1)
        self.conv3_3 = conv(dd[1]+dd[2],64,  kernel_size=3, stride=1)
        self.conv3_4 = conv(dd[2]+dd[3],64,  kernel_size=3, stride=1)
        self.predict_flow3 = self.predict_flow(dd[3]+dd[4])
        #self.deconv3 = deconv(2, 2, kernel_size=4, stride=2, padding=1) 
        #self.upfeat3 = deconv(od+dd[4], 2, kernel_size=4, stride=2, padding=1) 
        
        od = nd+64+2
        self.conv2_0 = conv(od,      512, kernel_size=3, stride=1)
        self.conv2_1 = conv(dd[0],   256, kernel_size=3, stride=1)
        self.conv2_2 = conv(dd[0]+dd[1],128,  kernel_size=3, stride=1)
        self.conv2_3 = conv(dd[1]+dd[2],64,  kernel_size=3, stride=1)
        self.conv2_4 = conv(dd[2]+dd[3],64,  kernel_size=3, stride=1)
        self.predict_flow2 = self.predict_flow(dd[3]+dd[4]) 
        #self.deconv2 = deconv(2, 2, kernel_size=4, stride=2, padding=1) 
        
        self.dc_conv1 = conv(dd[4]+2,  512, kernel_size=3, stride=1, padding=1,  dilation=1)
        self.dc_conv2 = conv(512,      256, kernel_size=3, stride=1, padding=2,  dilation=2)
        self.dc_conv3 = conv(256,      128, kernel_size=3, stride=1, padding=4,  dilation=4)
        self.dc_conv4 = conv(128,      64,  kernel_size=3, stride=1, padding=8,  dilation=8)
        self.dc_conv5 = conv(64,       32,  kernel_size=3, stride=1, padding=16, dilation=16)
    ...
    def forward(self, feature_list_1, feature_list_2, img_hw):
        c12, c13, c14, c15, c16 = feature_list_1
        c22, c23, c24, c25, c26 = feature_list_2
...
class PFlowNet(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.flowcalc = PWC_tf()
    def forward(self,x1, x2):
        out = self.flowcalc(x1, x2, (par.img_h, par.img_w))
        return out

if __name__ == '__main__':
    from torchvision.models import swin_v2_b
    model = swin_v2_b()
    inp = torch.rand(1,3,128,384)
    out = model(inp)
    print(out.shape)
